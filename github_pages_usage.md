...
* 본 워크스페이스에 설치된 파일들 (`index.html`, `README.md`, `_sidebar.md`, `.nojekyll`, 각 모듈 및 부록 마크다운 파일 등)이 온전히 있어야 합니다.
...
현재 작업 공간인 `/mnt/e/ai/pages` 폴더로 터미널을 열고 다음 명령어를 순서대로 실행합니다.
...
> 이를 방지하기 위해 생성해 둔 [**.nojekyll**](.nojekyll) 파일이 반드시 루트 경로에 함께 push되어야 정상 작동합니다.
